...
#									Created By: Aurora.php
#								  Social Media: IG Aurora.php
'''
Imports
'''
import sys
...
configFile = installDir + "/Aurora.cfg"
print(installDir)
config = ConfigParser.RawConfigParser()
config.read(configFile)

toolDir = installDir + config.get('Aurora', 'toolDir')
logDir = installDir + config.get('Aurora', 'logDir')
yes = config.get('Aurora', 'yes').split()
color_random=[color.HEADER,color.IMPORTANT,color.NOTICE,color.OKBLUE,color.OKGREEN,color.WARNING,color.RED,color.END,color.UNDERLINE,color.LOGGING]
random.shuffle(color_random)
Auroralogo = color_random[0] + '''
                                                         
                                                         
      db                                                 
     ;MM:                                                
    ,V^MM.  `7MM  `7MM  `7Mb,od8 ,pW"Wq.`7Mb,od8 ,6"Yb.  
   ,M  `MM    MM    MM    MM' "'6W'   `Wb MM' "'8)   MM  
   AbmmmqMA   MM    MM    MM    8M     M8 MM     ,pm9MM  
  A'     VML  MM    MM    MM    YA.   ,A9 MM    8M   MM  
.AMA.   .AMMA.`Mbod"YML..JMML.   `Ybmd9'.JMML.  `Moo9^Yo.
...
def agreement():
    while not config.getboolean("Aurora", "agreement"):
        ...
        if agree in yes:
            config.set('Aurora', 'agreement', 'true')

class Aurora:
    def __init__(self):
        clearScr()
        self.createFolders()
        print (Auroralogo + color.RED + '''
       }--------------{+} Coded By Aurora {+}--------------{
       }--------{+}  GitHub.com/Aurora/Aurora {+}--------{
    ''' + color.END + '''
       ...
       {4}--Sniffing & Spoofing
       {5}--Web Hacking & Attacks
       {6}--Private Web Hacking & Attacks
       {7}--Post Exploitation
       {8}--INSTALL & UPDATE
       {9}-CONTRIBUTORS
       {1337}-EXIT\n
     ''')
        choice = raw_input(AuroraPrompt)
        clearScr()
        if choice == "1":
    ...
    Yb      Yb   dP 88 Y88   88   88"Yb  88 88""Yb o.`Y8b
     ...
     ''')
        contributorsURL = 'https://api.github.com/repos/Aurora/Aurora/contributors'
    ...
    def update(self):
        os.system("git clone --depth=1 https://github.com/Aurora/Aurora.git")
        os.system("cd Aurora && bash ./update.sh")
        os.system("Aurora")
...
class sniffingSpoofingMenu:
    ...
    def __init__(self):
        ...
        choice6 = raw_input(AuroraPrompt)
        clearScr()
        if choice6 == "1":
            setoolkit()
        elif choice6 == "2":
...
class webHackingMenu:
    ...
    def __init__(self):
        ...
        print("   {99}-Back To Main Menu \n")
        choiceweb = raw_input(AuroraPrompt)
        clearScr()
        if choiceweb == "1":
            maine()
        elif choiceweb == "2":
...
class postExploitationMenu:
    ...
    def __init__(self):
        ...
        choice11 = raw_input(AuroraPrompt)
        clearScr()
        if choice11 == "1":
            sitechecker()
        elif choice11 == "2":
...
class informationGatheringMenu:
    ...
    def __init__(self):
        ...
        print("  {8}--Crips\n  ")
        print("  {99}-Back To Main Menu \n")
        choice2 = raw_input(AuroraPrompt)
        clearScr()
        if choice2 == "1":
            nmap()
        elif choice2 == "2":
...
class crips:
    ...
    def __init(self):
        ...
        self.gitRepo = "https://github.com/Aurora/Crips.git"
...
class wirelessTestingMenu:
    ...
    def __init__(self):
        ...
        choice4 = raw_input(AuroraPrompt)
        clearScr()
        if choice4 == "1":
            reaver()
        elif choice4 == "2":
...
class exploitationToolsMenu:
    ...
    def __init__(self):
        ...
        choice5 = raw_input(AuroraPrompt)
        clearScr()
        if choice5 == "1":
            atscan()
        elif choice5 == "2":
